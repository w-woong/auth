...
type TokenUsc interface {
	...
	// ValidateIDToken(ctx context.Context, tokenIdentifier string, idTokenStr string) (dto.Token, error)
	...
	// Refresh(ctx context.Context, tokenID string) (dto.Token, bool, error)
	...
	SaveToken(ctx context.Context, w http.ResponseWriter, token *oauth2.Token) (dto.Token, error)
	...
	RegisterUser(ctx context.Context, tokenID string, claims validators.IDTokenClaims) (userdto.User, error)
